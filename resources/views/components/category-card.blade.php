...
@else
    <div class="{{ $cardClasses }}">
        <div class="p-8 text-center flex flex-col items-center space-y-6">
            <div class="inline-flex items-center justify-center {{ $iconSize }} bg-gradient-to-br from-amazon-verde-500 to-amazon-verde-600 rounded-2xl group-hover:scale-110 transition-transform duration-300">
                <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path>
                </svg>
            </div>
            <h3 class="{{ $titleSize }} font-bold text-gray-900" style="color: var(--card-title-color) !important;">{{ $category->name }}</h3>
            <p class="text-gray-600 leading-relaxed line-clamp-2 min-h-[48px]" style="color: var(--card-text-color) !important;">
                {{ $category->description ?: __('frontend.category_card.default_description') }}
            </p>
            <div class="inline-flex items-center bg-amazon-verde-50 text-amazon-verde-700 px-5 py-2 rounded-full text-sm font-semibold">
            ...
            <a href="{{ route('frontend.products', ['category' => $category->id]) }}" 
               class="inline-flex items-center justify-center bg-gradient-to-r from-amazon-verde-600 to-amazon-verde-700 text-white px-10 py-3 rounded-xl font-semibold hover:from-amazon-verde-700 hover:to-amazon-verde-800 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 min-w-[220px]">
                {{ __('frontend.common.see_products') }}
